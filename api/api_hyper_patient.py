...
class ApiHyperPatient:
    ...
    def __init__(self):
        ...
        todo 初始化
        '''
        logger.info('开始获取实时同步高血压高危、疑似接口的url地址：')
        # 构造url地址
        self.url = JCGW_IP + ':' + PORT3 + '/icdm-controller/yfrhApi/syncHyperPatient'
        logger.info('实时同步高血压高危、疑似接口的url地址为：{}'.format(self.url))

    def hyper_patient(self, session):
        # 入参
        data = {
        ...
        logger.info('准备发起实时同步高血压高危、疑似接口的请求，请求的header是：{},请求的参数是：{}'.format(HEADERS, data))
        # 发起请求
