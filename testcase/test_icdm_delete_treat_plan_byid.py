...
from api.api_delete_treat_plan_byid import ApiDeleteTreatPlanByid


@allure.feature('治疗方案模块')
class TestDeleteTreatPlanByid:
    # 在所有的测试用例之前创建session
    ...
    @allure.story('用户正常登录后，删除治疗方案')
    @allure.title('删除治疗方案接口的测试用例')
    def test_delete_treat_plan_byid(self):
        '''
        测试是否能够删除治疗方案（支持批量）
        前置：成功获取token
        '''
        # 发起请求
        response = self.delete_treat_plan_byid_object.delete_treat_plan_byid(self.session).json()
        # 断言
