...
from api.api_diabetes_confirm import ApiDiabetesConfirm


@allure.feature('第三方实时同步模块')
class TestDiabetesConfirm:
    # 在所有的测试用例之前创建session
    ...
    @allure.story('用户正常登录后，第三方实时同步糖尿病确诊接口')
    @allure.title('实时同步高血压确诊接口的测试用例')
    def test_diabetes_confirm(self):
        '''
        测试第三方接口是否能够实时同步糖尿病确诊患者
        前置：成功获取token
        '''
        # 发起请求
        response = self.diabetes_confirm_object.diabetes_confirm(self.session).json()
        # 断言
